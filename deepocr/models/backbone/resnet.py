...
class AsterBlock(nn.Module):
    """Slightly modified version of ResNet block."""
...
class ResNetAster(nn.Module):
    ...
    def __init__(self, in_channels=1, batch_norm=True, frozen_stages=-1):
        ...
        if batch_norm:
            self.layer0.add_module("bn1", nn.BatchNorm2d(32))
    ...
    def make_layer(self, inplanes, planes, blocks, stride, batch_norm):
        downsample = False
        if stride != [1, 1] or inplanes != planes:
            if batch_norm:
                ...
                downsample = conv1x1(inplanes, planes, stride)

        layers = []
        layers.append(
            AsterBlock(inplanes, planes, stride, downsample, batch_norm=batch_norm)
        )
        inplanes = planes
        for _ in range(1, blocks):
            layers.append(AsterBlock(inplanes, planes, batch_norm=batch_norm))
